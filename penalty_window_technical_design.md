# Penalty Window — Technical Design
...
|---|---|---|
| I1 | 3.2 | No operation in `coach_engine`/`recovery_plan_generator` may write to `penalty_windows.status`, `base_duration_hours`, or `extensions_hours`. Writing to these fields is reserved exclusively for `penalty_engine`. |
...
| I11 | 3.8 | `Incident.consumed_by_penalty_window_id` is written exactly once during the lifetime of an Incident (a write-once column — a second write attempt is an error, not a silent overwrite). |
| I12 | 3.8 | Selecting available Incidents for a new window always filters on `consumed_by_penalty_window_id IS NULL AND created_at > <closed_at of the last closed window>`. |
...
| I25 (API-BOUNDARY-2) | 2.6, Hygiene Privilege module | No module other than the Penalty Engine reads `incidents.trust_domain`/`consumed_by_penalty_window_id` directly. The only permitted path to learn which Trust domain(s) a Penalty Window relates to is `get_penalty_window_relevant_domains()` (2.6). |
...
| `ACTIVE → ACTIVE` (extend) | new Incident while `ACTIVE`, `should_extend() == True` | `extensions_hours += Δ` (implicitly capped via `target_active_hours`); emits `EXTENDED`, `INCIDENT_CONSUMED` |
...
hygiene (`philosophy.md` 3.9). This information lives in the `incidents`
table (`trust_domain`, `consumed_by_penalty_window_id`), which is
internal to the Penalty Engine for the same reason `freeze_periods` is:
opening direct read access to it would let module boundaries blur at
...
def get_penalty_window_relevant_domains(db: Database, penalty_window_id: str) -> frozenset[str]:
    ...
    `trust_domain` values of the Incidents consumed by it
    (`consumed_by_penalty_window_id = penalty_window_id`).
    ...
    (severity, description, confirmation, etc.) — only the set of
    domain identifiers, which is the minimum a consumer legitimately
    needs to decide "is this window related to domain X."
    """
```
...
CREATE TABLE incidents (
    ...
    consumed_by_penalty_window_id TEXT   -- write-once, see I11
);
...
CREATE TABLE domain_event_consumers (
    ...
    consumer_name   TEXT NOT NULL,       -- 'recovery_plan_generator' | 'activity_authorization' | ...
...
| `penalty_window.extended` | penalty_engine | a new Incident extended the window |
...
| `incident.consumed` | penalty_engine | an Incident is assigned to the window |
| `recovery_credit.earned` | penalty_engine | a write to the ledger |
| `emergency_override.triggered` | hygiene_access / discord_bot (the shortest path, see I16) | immediately on calling `emergency_freeze()` |
| `recovery_plan.created` | recovery_plan_generator | a reaction to `started` |
| `recovery_plan.frozen` | recovery_plan_generator | a reaction to `frozen` |
| `recovery_plan.resumed` | recovery_plan_generator | a reaction to `resumed` |
| `recovery_plan.completed` | recovery_plan_generator | a reaction to `completed` |
| `penalty_engine.freeze_expired` | penalty_engine | automatic closure of a freeze whose `expires_at` has passed (4.5) — consumed by Activity Authorization |
...
(`ended_at = now`) and emits `penalty_engine.freeze_expired` (payload:
`authorization_decision_id`, `freeze_period_id`) — Activity Authorization
consumes this event to close the corresponding session/decision (see
`activity_authorization_technical_design.md`, the crash recovery
section). This applies regardless of whether the closure was triggered
during normal operation or during startup reconciliation after an
outage — the same code path, the same event.
...
| T11 | An Incident during ACTIVE | `ACTIVE` | a new Incident is registered | `consumed_by_penalty_window_id` set to the current window; if `should_extend()`, an extension too (I11, I12) |
...
| T13 | An attempt to reuse a consumed Incident | an Incident with `consumed_by_penalty_window_id != NULL` | an attempt to include it in a new window | rejected at the `get_available_incidents()` query level (I12); bonus test: a direct attempt at a second `UPDATE` of `consumed_by_penalty_window_id` fails/is blocked (I11, write-once) |
| T14 | An attempt to start a new window using an old Incident after the maximum has been reached | window A closed at 336h with old Incident I | an attempt to create window B with Incident I | rejected — `I.consumed_by_penalty_window_id` is no longer NULL (I12) |
...
3. **`should_extend()`** — the general principles (proportionality,
   explainability, auditability, absolute maximum, targeting by Trust
   domain) are now anchored in `philosophy.md` 3.8 (v1.3). Per the agreed
   process, the concrete algorithm is a separate step **after** the
   invariants, state machine, persistence, and events are closed out —
   that is, the next step after this document, not part of it.
