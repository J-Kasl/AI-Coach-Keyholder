# Activity Authorization — Technical Design (v4)

> Draft for review, **not implemented**. Based on `philosophy.md` v1.5
...
| `activity_authorization.freeze_confirmed` | activity_authorization (a consumer of an event from the Penalty Engine) | `PENDING_FREEZE → ACTIVE` (8.4) — the payload carries `expires_at` (the single source of truth, fix for Point 10) |
...
| `activity_authorization.resume_confirmed` | activity_authorization (a consumer of an event from the Penalty Engine) | `PENDING_RESUME → CLOSED` after confirmation that `freeze_periods` was closed (8.5) |
| `penalty_engine.freeze_expired` | *(consumed, not emitted by this module — see penalty_window_technical_design.md 4.5)* | automatic expiration initiated by the Penalty Engine — Activity Authorization reacts by closing the session as `EXPIRED` and the decision `→ CLOSED` (8.5) |
...
reading the table directly. It is called mandatorily at startup (16.2),
not as an optional periodic audit.
...
5. **Startup reconciliation is a single-writer operation**, protected
   by a restart-safe database lease (16.2), and all time in the system
   comes from a single injected `Clock` (16.7) — both so that an
...
### 16.2 Startup Reconciliation — the Entry Point, Protected by a Single-Writer Lease

```python
STARTUP_LEASE_DURATION = timedelta(minutes=5)   # parameter, with margin above the expected recovery duration

def on_process_startup(db: Database, process_id: str, clock: Clock) -> None:
    """
    Called BEFORE the Discord bot starts / before the first request is
    accepted.

    First acquires a restart-safe database leadership lease (fix for
    "startup must be single-writer") — if two process instances are
    accidentally running at once, only one may perform reconciliation.
    The lease is a database row with an expiration, not an in-memory
    mutex or a PID file — neither would reliably catch a restart or a
    double launch.
    """
    now = clock.now()
    ...
    if lease is None:
        ...
        # performed recovery. This process does not participate in startup
        # as a reconciler — it either waits, or (if this is an
    ...
    try:
        # The order is deliberate: first the Penalty Window (because
        # Activity Authorization depends on its state), then Activity
        # Authorization, then the outbox publisher (so that recovery
        # steps from the previous two steps have a chance to be
        # delivered right away, rather than waiting for the next cycle).
        recover_penalty_window_state(db, now)          # see penalty_window_technical_design.md 4.5
        recover_activity_authorization_state(db, now)  # see 16.3 below
        publish_pending_outbox_events(db, process_id, now)  # I23/4.6 — claim + delivery
    finally:
...
def acquire_system_startup_lease(db: Database, process_id: str, now: datetime, duration: timedelta) -> Lease | None:
    """
    An atomic UPDATE/INSERT over the single-row system_startup_lease table:
...
`recover_penalty_window_state()` is simply `ensure_current_state()`
called at startup (Penalty Window document, 4.5) — no new function,
just an explicit call at a new location. Both recovery functions
(Penalty Window and Activity Authorization) run **inside** a valid
startup lease — they do not need their own protection against
concurrent execution; the surrounding `on_process_startup()` provides
that.
...
| RT9 | A repeated restart causes no duplicate effect | `on_process_startup()` run 2x or 10x in a row with no activity in between | The same result as after a single run — no second `SPEND`/`REVERSAL`/`freeze_periods` row (idempotency of all recovery steps) |
...
| RT11 | Two instances run recovery simultaneously | Two processes call `on_process_startup()` at nearly the same time | Only one acquires `system_startup_lease`; the other gets `StartupLeaseNotAcquired` and does not perform reconciliation — no concurrent write from both sides (LEASE-1) |
...
The `Clock` instance is injected into `on_process_startup()`,
