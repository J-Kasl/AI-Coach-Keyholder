...
class TestSystemMessageIsFirstAndDeterministic:
    def test_first_message_is_system_role(self) -> None:
        request = build_generation_request(snapshot=_snapshot(), plan=_plan(), working_memory_turns=(), max_output_characters=1800)
    ...
    def test_system_message_content_is_identical_regardless_of_user_message_content(self) -> None:
        ...
        for msg in adversarial_messages:
            request = build_generation_request(
                snapshot=_snapshot(current_user_message=msg), plan=_plan(), working_memory_turns=(), max_output_characters=1800,
            )
...
class TestCurrentUserMessageIsAlwaysLastAndSeparate:
    def test_current_message_is_the_final_message_with_user_role(self) -> None:
        request = build_generation_request(snapshot=_snapshot("what's up"), plan=_plan(), working_memory_turns=(), max_output_characters=1800)
...
class TestRecentHistoryInsertedAsSeparateTurns:
    def test_recent_messages_appear_between_system_and_current_message(self) -> None:
        recent = (
            WorkingMemoryTurn(role=WorkingMemoryRole.USER, content="earlier question"),
            WorkingMemoryTurn(role=WorkingMemoryRole.ASSISTANT, content="earlier answer"),
        )
        request = build_generation_request(snapshot=_snapshot("now"), plan=_plan(), working_memory_turns=recent, max_output_characters=1800)
        assert request.messages[1].role == ModelMessageRole.USER
...
class TestMaxOutputCharactersPassedThrough:
    def test_max_output_characters_is_forwarded(self) -> None:
        request = build_generation_request(snapshot=_snapshot(), plan=_plan(), working_memory_turns=(), max_output_characters=42)
