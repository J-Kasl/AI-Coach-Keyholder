# Trust Manager — Technical Design (v4)

> **v4 — applied the Goal Management integration proposed in
> `goal_technical_design.md` 11.1/11.2 (Section 15):** `GOAL_PROGRESS`/
> `GOAL_SETBACK` added to `EvidenceType`; `record_goal_assessment_evidence()`
> reads a completed `GoalAccountabilityAssessment` via Goal Management's
> own narrow API and decides independently whether to write
> `TrustEvidence`; `TrustEvidence` now enforces
> `UNIQUE(source_entity_type, source_entity_id, evidence_type)` (TI25),
> protecting every evidence-producing integration in this module against
> duplicate writes from at-least-once event redelivery.

> Status: **Architecture baseline — approved for implementation.**
> Reached this status once the `Incident` ownership fix (Finding 1), the
> crash recovery gap fix (Finding 5), and the Goal Management
> integration (Section 15) were all applied — this document is now the
> baseline for the Trust Manager's implementation, not a proposal still
> awaiting changes.
>
> Based on `philosophy.md` v1.3, and replaces v1 of this document —
> incorporates 8 points from review
...
>
> **v3 — fixes from the `system_state_machine.md` integration audit
> (Findings 1 and 5):** (1) `Incident.consumed_by_penalty_window_id`
> removed — `Incident` is now owned exclusively by the Trust Manager
> (confirmation, assessment, description), with two new narrow public
> read functions (`get_incident_assessment()`,
> `get_confirmed_incidents_since()`, Section 13) replacing what was
> previously an incompatible second schema implied by
> `penalty_window_technical_design.md`; consumption bookkeeping now
> lives entirely in that document's own `incident_consumption` table;
> (2) `confirm_incident()` now writes the `ConfirmationRecord`,
> `Incident.assessment`, and the resulting `TrustEvidence` in a single
> transaction when reaching `CONFIRMED` (Section 14), closing a
> long-identified crash-recovery gap, with a corresponding startup
> reconciliation step.
>
...
class EvidenceType(StrEnum):
    ...
    GOAL_PROGRESS = "goal_progress"           # see Section 15 — applied per goal_technical_design.md 11
    GOAL_SETBACK = "goal_setback"              # see Section 15
```

`TrustEvidence` additionally carries an uncommented-on but load-bearing
constraint, now made explicit (fix, Section 15):
`UNIQUE(source_entity_type, source_entity_id, evidence_type)`. One raw
source fact produces at most one piece of evidence of a given type —
this is what makes redelivery of the same upstream event safe across
every evidence-producing integration this system has, not only the
Goal one added in Section 15 (TI25).
...
class Incident:
    ...
    all descriptive fields. Does NOT track which Penalty Window (if any)
    consumed it (fix for the "two incompatible schemas" finding, System
    State Machine Finding 1) -- that bookkeeping belongs to the Penalty
    Engine's own incident_consumption table (see
    penalty_window_technical_design.md), which references this
    Incident's id but never duplicates its confirmation/assessment
    shape, and which the Trust Manager never reads or writes.
    """
    id: str
    created_at: datetime
    rule_group_id: str
...
- `UNCONFIRMED`/`PROVISIONAL` may create an `ObservationRecord` or a
  ...
  **never** eligible for consumption by a Penalty Window (a concern of
  the Penalty Engine's own `incident_consumption` table, which only
  ever references `CONFIRMED` Incidents), **never** an input into
...
already consumed by an earlier, closed Penalty Window — a fact tracked
by the Penalty Engine's own `incident_consumption` table
(`penalty_window_technical_design.md`), never by the Trust Manager
(see the note on `Incident`, 2.10). This is intentional and does not
conflict with the single-use rule (`philosophy.md` 3.8):

- **Use as a consumed Incident** (grounds for starting/extending a
  specific Penalty Window) is governed entirely by the Penalty Engine's
  `incident_consumption` table, which the Trust Manager never reads or
  writes — this remains exactly once during the lifetime of an
  Incident, enforced there, not here.
- **Use as historical evidence of a pattern for Trust** (an entry in
  `source_incident_ids`) is a read-only, non-destructive reference
  within the Trust Manager's own domain — it consumes nothing; it
  merely says "this is not the first time." It requires no knowledge of
  whether, or by which window, the Incident was ever consumed.

At the type level, this distinction is enforced by `RepetitionEvidence`
being a read-only DTO (a frozen dataclass assembled by a query) that
never references the Penalty Engine's consumption bookkeeping at all —
the two facts (confirmed-and-repeated, for Trust; consumed-by-a-window,
for Penalty Window) are tracked independently, by their respective
owning modules, and neither reads the other's table.
...
| `incident.confirmation_changed` | trust_manager | a new `ConfirmationRecord` — **the ONE canonical event for every confirmation transition** (`UNCONFIRMED→PROVISIONAL`, `PROVISIONAL→CONFIRMED`, etc.), payload carries `previous_confirmation`/`new_confirmation`. Consumers that only care about a specific transition (e.g. the Penalty Engine, which acts only once `new_confirmation == CONFIRMED`) filter on the payload — there is deliberately no second, narrower event for "just reached CONFIRMED" (resolved via `docs/architecture/domain_events_catalog.md` Finding 1: one event, one canonical definition, filtered by payload, not duplicated as two event types for one underlying write). |
...
(both long since consumed by a closed Penalty Window, per the Penalty
Engine's own `incident_consumption` table) plus one new, just-confirmed
one. `RepetitionEvidence(same_rule_confirmed_count=3,
evaluation_window_days=30, source_incident_ids=(old1, old2, new))`.
`_repetition_contribution()` returns `1` (because `3 <= 3`). Nothing
about old1/old2 changes in the Trust Manager's own tables — they are
only read as evidence of a pattern, not re-consumed, and the Trust
Manager has no field to change even if it wanted to (that bookkeeping
lives entirely in the Penalty Engine).
...
| TT7 | A historical, consumed Incident as repetition evidence | Incident from a closed Penalty Window, present in the Penalty Engine's `incident_consumption` table | included in the `source_incident_ids` of a new `RepetitionEvidence` | allowed — the Trust Manager has no consumption field to change, since it never tracked that fact to begin with (5.4) |
...
## 13. Public Read API for Cross-Module Queries (v3 — Fix for System State Machine Finding 1)

`system_state_machine.md`'s integration audit identified that `Incident`
previously existed in two incompatible shapes: this document's full
model (confirmation, assessment) and a separate, thinner representation
implied by `penalty_window_technical_design.md`'s own `incidents`
table (a direct `severity` column, a `consumed_by_penalty_window_id`
field). This section resolves it: **the Trust Manager owns `Incident`
in full — confirmation, assessment, and all descriptive fields — and
exposes exactly two narrow read functions for the Penalty Engine.**
Consumption bookkeeping (which window consumed which Incident) moves
entirely to the Penalty Engine's own `incident_consumption` table (see
`penalty_window_technical_design.md`), which references an Incident's
`id` but never duplicates its shape.

```python
def get_incident_assessment(db: Database, incident_id: str) -> IncidentAssessment | None:
    """
    The ONLY permitted way for another module (specifically, Extension
    -- see extension_technical_design.md Section 4) to read an
    Incident's severity/cooperation data. Returns None if the Incident
    is not CONFIRMED (mirrors TI15 -- assessment does not exist before
    then) or does not exist. Exposes nothing about confirmation
    mechanics, ConfirmationRecord history, or TrustEvidence -- only the
    IncidentAssessment a CONFIRMED Incident already carries.
    """
...
class ConfirmedIncidentSummary:
    ...
    Incident/IncidentAssessment. Exposes only what the Penalty Engine
    needs to decide whether to start a new Penalty Window (2.2's
    '(none) -> ACTIVE' guard, '>=1 unconsumed Incident').
    """
    id: str
    trust_domain: str
    created_at: datetime
...
def get_confirmed_incidents_since(db: Database, since: datetime) -> list[ConfirmedIncidentSummary]:
    """
    The ONLY permitted way for the Penalty Engine to enumerate
    CONFIRMED Incidents when checking whether any are not yet present
    in its own incident_consumption table. Never exposes assessment,
    confirmation history, or anything beyond the three fields above.
    """
```

Both functions are read-only and narrow, following the same discipline
as `get_authorization_freeze_state()` and
`get_penalty_window_relevant_domains()`
(`penalty_window_technical_design.md` 2.5/2.6) — a category of function
this system now uses consistently at every cross-module boundary,
rather than ever exposing a raw table or a second writer.

---

## 14. Crash/Restart Recovery (Fix for System State Machine Finding 5)

### 14.1 The Gap

`confirm_incident()` (writing a `ConfirmationRecord` and updating
`Incident.confirmation`, Section 5.1) and the subsequent
`assess_severity()` call plus `TrustEvidence` write (Section 9) were
previously described as sequential steps, not necessarily one
transaction. A crash between them would leave a `CONFIRMED` Incident
with `assessment=None` indefinitely — silently invisible to Extension
(`get_incident_assessment()` would return `None` forever, since nothing
would ever complete the assessment) and to the Trust Manager's own
recalculation pipeline. Every other module in this system now has a
Section 16/8/9-style recovery mechanism; this was the one remaining
gap.

### 14.2 The Fix: One Transaction When Confirmation Reaches `CONFIRMED`

```python
def confirm_incident(db: Database, incident_id: str, new_confirmation: IncidentConfirmation, source: ConfirmationSource, evidence_description: str, now: datetime) -> None:
    """
    Updated (v3): when new_confirmation == CONFIRMED, the
    ConfirmationRecord write, the Incident.confirmation update, the
    assess_severity() call, and the resulting TrustEvidence write all
    happen in ONE transaction -- the same _apply_transition discipline
    used everywhere else in this system. For UNCONFIRMED -> PROVISIONAL
    (no assessment yet), only the first two steps occur, as before.
    """
    with db._connect() as conn:
        record = _build_confirmation_record(conn, incident_id, new_confirmation, source, evidence_description, now)
        _insert_confirmation_record(conn, record)
        _update_incident_confirmation(conn, incident_id, new_confirmation)
        _write_event(conn, _confirmation_changed_event(record))

        if new_confirmation == IncidentConfirmation.CONFIRMED:
            evidence_input = _load_incident_evidence(conn, incident_id)
            severity = assess_severity(evidence_input)
            assessment = _build_incident_assessment(severity, evidence_input, now)
            _update_incident_assessment(conn, incident_id, assessment)   # SAME transaction
            trust_evidence = _record_evidence_in_transaction(conn, incident_id, assessment, now)  # SAME transaction
            _write_event(conn, _evidence_recorded_event(trust_evidence))  # SAME transaction
```

This directly satisfies TI12 (state change and its `domain_event` in
the same transaction) applied to the one place it was previously
missing.

### 14.3 Startup Reconciliation

```python
def recover_trust_manager_state(db: Database, now: datetime) -> None:
    """
    Called from on_system_startup() (system_state_machine.md Section 7),
    as step 1 -- before Penalty Engine recovery, since Extension reads
    Incident.assessment via get_incident_assessment() and must never
    see a stale, incomplete result.

    With the fix in 14.2, this anomaly can no longer be CREATED going
    forward, but existing data (or a crash during a version of the code
    predating this fix) could still contain it -- this function detects
    and repairs it idempotently.
    """
    for incident in db.get_confirmed_incidents_with_null_assessment():
        evidence_input = _load_incident_evidence(db, incident.id)
        severity = assess_severity(evidence_input)
        assessment = _build_incident_assessment(severity, evidence_input, now)
        with db._connect() as conn:
            _update_incident_assessment(conn, incident.id, assessment)
            trust_evidence = _record_evidence_in_transaction(conn, incident.id, assessment, now)
            _write_event(conn, _evidence_recorded_event(trust_evidence))
```

Idempotent by construction: `get_confirmed_incidents_with_null_assessment()`
returns nothing once every such Incident has been repaired, so running
this function multiple times in a row is safe (the same discipline
used by every other module's recovery function in this system).

### 14.4 New Invariants

| # | Source | Invariant |
|---|---|---|
| TI22 (API-BOUNDARY-4) | 13, System State Machine Finding 1 | `Incident` (existence, confirmation, assessment, description) is owned exclusively by the Trust Manager. No other module reads or writes it directly — only via `get_incident_assessment()` and `get_confirmed_incidents_since()`. Consumption bookkeeping is never stored on `Incident` — it lives entirely in the Penalty Engine's own `incident_consumption` table. |
| TI23 | 14.2, System State Machine Finding 5 | When `confirm_incident()` transitions an Incident to `CONFIRMED`, the `ConfirmationRecord` write, the `Incident.confirmation`/`assessment` updates, and the resulting `TrustEvidence` write occur in a single transaction. No `CONFIRMED` Incident may persist with `assessment=None` beyond a single, atomic write boundary. |
| TI24 | 14.3 | `recover_trust_manager_state()` runs before Penalty Engine recovery in the system startup sequence (`system_state_machine.md` Section 7), and is idempotent — repeated runs produce the same result as a single run. |

### 14.5 Test Additions

| # | Scenario | Given | When | Then |
|---|---|---|---|---|
| TT25 | Confirmation and assessment are atomic | `confirm_incident(..., CONFIRMED, ...)` in progress | a simulated crash between the `Incident.confirmation` update and the `TrustEvidence` write | the whole transaction rolls back — `confirmation` reverts to its prior value; no `CONFIRMED` Incident with `assessment=None` results (TI23) |
| TT26 | Startup recovery repairs a pre-existing anomaly | a `CONFIRMED` Incident with `assessment=None` (simulating data from before the TI23 fix) | `recover_trust_manager_state()` runs | the Incident receives a complete `assessment` and corresponding `TrustEvidence`; running recovery again is a no-op (TI24) |
| TT27 | `get_incident_assessment()` respects confirmation gating | an `UNCONFIRMED`/`PROVISIONAL` Incident | `get_incident_assessment()` | returns `None` (TI15, TI22) |
| TT28 | No module bypasses the read API | any external module's code path | inspect all reads of Incident data | only `get_incident_assessment()` and `get_confirmed_incidents_since()` are used; no direct table access (TI22) |

---

## 15. Goal Accountability Assessment Integration (Applying `goal_technical_design.md` 11.1/11.2)

Previously proposed, not applied. Now applied, per your approval: this
is the second of two pending integrations, both following the identical
pattern — a source domain publishes a completed, self-contained
judgment; this module (the domain that owns `TrustEvidence`) reads it
through a narrow function and decides, independently, whether and how
to write its own evidence.

```
Goal Management
    -- owns GoalEvaluation (Coach's development interpretation)
    -- owns GoalAccountabilityAssessment (Keyholder's accountability interpretation)
    -- publishes a domain event once a GoalAccountabilityAssessment is recorded
Trust Manager (this document)
    -- consumes the event
    -- reads the assessment via Goal Management's own narrow API
    -- decides whether relevant_to_trust and what direction imply
    -- writes TrustEvidence itself, or writes nothing
```

### 15.1 `record_goal_assessment_evidence()`

```python
def record_goal_assessment_evidence(db: Database, assessment_id: str, event: DomainEvent) -> TrustEvidence | None:
    """
    Reads the referenced GoalAccountabilityAssessment via Goal
    Management's own read API (the same cross-module read discipline
    used for get_authorization_freeze_state()/
    get_penalty_window_relevant_domains() elsewhere in this system --
    Goal Management exposes its own equivalent, not a raw table).

    Writes TrustEvidence ONLY if assessment.relevant_to_trust is True
    AND assessment.direction is PROGRESS or SETBACK -- never for
    NEUTRAL, and never when relevant_to_trust is False. direction=PROGRESS
    maps to evidence_type=GOAL_PROGRESS; direction=SETBACK maps to
    GOAL_SETBACK.

    UNIQUE(source_entity_type, source_entity_id, evidence_type) (2.3/2.4)
    -- with source_entity_type='goal_accountability_assessment',
    source_entity_id=assessment_id -- makes this function naturally
    idempotent against redelivery of the same event: a second call for
    the same assessment_id either fails the uniqueness constraint (if
    called directly a second time) or is a no-op after the
    domain_event_consumers check (I19) catches the redelivered event
    first, whichever layer sees it first. Both layers exist
    deliberately -- consumer-side dedup (I19) protects against a
    redelivered event.id; the TrustEvidence uniqueness constraint (TI25)
    protects against the same assessment_id being credited twice even
    via two DIFFERENT event deliveries (e.g., a bug that publishes a
    second, distinct event for the same assessment).
    """
    with db._connect() as conn:
        assessment = goal_management.get_accountability_assessment(conn, assessment_id)  # Goal Management's own narrow read API

        if not assessment.relevant_to_trust or assessment.direction == GoalAccountabilityDirection.NEUTRAL:
            return None  # a legitimate, expected outcome -- most assessments produce no Trust effect

        evidence_type = (EvidenceType.GOAL_PROGRESS
                          if assessment.direction == GoalAccountabilityDirection.PROGRESS
                          else EvidenceType.GOAL_SETBACK)

        evidence = TrustEvidence(
            id=new_id(), domain_id=assessment.trust_domain, created_at=utc_now(),
            evidence_type=evidence_type,
            source_entity_type='goal_accountability_assessment', source_entity_id=assessment_id,
            raw_weight=_goal_evidence_weight(assessment),   # a deterministic, table-driven weight -- same discipline as assess_severity(), never a free-form number (TI14-style)
            evidence_confidence=_goal_evidence_confidence(assessment),
            explanation=assessment.rationale,
        )
        _insert_trust_evidence(conn, evidence)   # UNIQUE constraint enforced here (TI25)
        _write_event(conn, _evidence_recorded_event(evidence))
        return evidence
```
...
| # | Source | Invariant |
|---|---|---|
| TI21 | `goal_technical_design.md` 11.2 | `GOAL_PROGRESS`/`GOAL_SETBACK` `TrustEvidence` is written only via `record_goal_assessment_evidence()`, called only after a `GoalAccountabilityAssessment` exists. No code path writes `GOAL_PROGRESS`/`GOAL_SETBACK` directly from a `GoalEvaluation` or raw `GoalEvidence`. |
| TI25 | 2.3/2.4, 15.1 | `TrustEvidence` enforces `UNIQUE(source_entity_type, source_entity_id, evidence_type)`. A redelivered event, or a duplicate call for the same source fact, never creates a second piece of evidence — this applies to every evidence-producing integration this module has, not only Section 15's. |

### 15.3 Test Additions

| # | Scenario | Given | When | Then |
|---|---|---|---|---|
| TT29 | Not relevant to Trust produces no evidence | `GoalAccountabilityAssessment(relevant_to_trust=False)` | `record_goal_assessment_evidence()` | returns `None`; no `TrustEvidence` written |
| TT30 | Neutral direction produces no evidence | `GoalAccountabilityAssessment(relevant_to_trust=True, direction=NEUTRAL)` | `record_goal_assessment_evidence()` | returns `None`; no `TrustEvidence` written |
| TT31 | Progress produces `GOAL_PROGRESS` evidence | `GoalAccountabilityAssessment(relevant_to_trust=True, direction=PROGRESS)` | `record_goal_assessment_evidence()` | a `TrustEvidence(evidence_type=GOAL_PROGRESS)` is written |
| TT32 | Setback produces `GOAL_SETBACK` evidence | `GoalAccountabilityAssessment(relevant_to_trust=True, direction=SETBACK)` | `record_goal_assessment_evidence()` | a `TrustEvidence(evidence_type=GOAL_SETBACK)` is written |
| TT33 | Redelivery produces at most one evidence row | a `GoalAccountabilityAssessment` already processed into `TrustEvidence` | the same event is redelivered (at-least-once, `penalty_window_technical_design.md` I23) and `record_goal_assessment_evidence()` runs again for the same `assessment_id` | the consumer-side dedup (I19) catches it first in the normal path; even bypassing that, the `UNIQUE(source_entity_type, source_entity_id, evidence_type)` constraint (TI25) rejects a second insert — at most one `TrustEvidence` row ever exists for this assessment |
